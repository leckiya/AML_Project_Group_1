...
# 4. Train/validation split (fixed)
...
# 7. Training (once entire learning)
plain_model = build_plain_model()
plain_model.compile(optimizer=Adam(1e-4), loss='mse')
early = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
plain_model.fit(dstrain, validation_data=dsval, epochs=50, callbacks=[early], verbose=2)

# 8. Evaluation and result restoration
y_val_pred = plain_model.predict(dsval)
...
plain_model.save('plain_model.h5') 