...
class NutriNet(nn.Module):
    """
    NutriNet Architecture for nutrition prediction
    Built from scratch with modern deep learning practices
    """
    def __init__(self, num_outputs=5):
        super(NutriNet, self).__init__()
        ...
        # Classification head for nutrition prediction
    ...
    def _initialize_weights(self):
        """Initialize weights using modern initialization techniques"""
...
class ResidualBlock(nn.Module):
    """Residual block with proper skip connections"""
...
def test_specific_dishes(model, dataset, device, dish_ids=['dish_1565033189', 'dish_1556572657'], output_file="nutrinet_result/specific_dish_test_results.txt"):
    """
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("NUTRINET SPECIFIC DISH TEST RESULTS\n")
        f.write("=" * 80 + "\n")
    ...
    print(f"\nNutriNet specific dish test results saved to: {output_file}")

def train_model(model, train_loader, val_loader, num_epochs=50, device='cuda', norm_factors=None, norm_type=None):
    """Train the nutrition prediction model for a fixed number of epochs, with progress bars and checkpoints"""
    ...
    os.makedirs('nutrinet_checkpoint', exist_ok=True)
    print(f"Starting NutriNet training for {num_epochs} epochs...")
    print(f"Checkpoints will be saved every 10 epochs to the 'nutrinet_checkpoint/' directory")
    print("=" * 60)
    for epoch in range(num_epochs):
        ...
        train_acc = 0.0
        batch_count = 0
        train_pbar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs} [Train]', leave=True, dynamic_ncols=True)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'nutrinet_checkpoint/best_nutrinet_model.pth')
            print(f'New best NutriNet model saved! Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.1f}%')
        ...
        if (epoch + 1) % 10 == 0:
            ...
            checkpoint_path = f'nutrinet_checkpoint/checkpoint_epoch_{epoch+1}.pth'
            torch.save(checkpoint, checkpoint_path)
            print(f'NutriNet checkpoint saved: {checkpoint_path}')
    model.load_state_dict(torch.load('nutrinet_checkpoint/best_nutrinet_model.pth'))
    print(f'NutriNet training completed! Best validation loss: {best_val_loss:.4f}')
...
def save_metrics_summary(val_metrics):
    ...
    with open('nutrinet_result/nutrinet_training_metrics_summary.txt', 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("NUTRINET TRAINING METRICS SUMMARY\n")
        f.write("=" * 60 + "\n\n")
    ...
    print("NutriNet metrics summary saved to: nutrinet_result/nutrinet_training_metrics_summary.txt")
...
def save_evaluation_metrics(metrics, output_file, predictions=None, targets=None, denorm_fn=None):
    ...
    print(f"Final evaluation metrics saved to: {output_file}")
...
def main():
    ...
    print("=" * 60)
    print("NutriNet Food Nutrition Prediction - RGB Images Only")
    print("=" * 60)
    print(f"Image directory: {image_dir}")
    print(f"Metadata file: {metadata_file}")
    print("Only rgb.png files will be used for training")
    print("(depth_color.png and depth_raw.png are ignored)")
    print("Using NutriNet architecture with residual connections")
    print("=" * 60)
    ...
    # Create dataset (RGB images only)
    ...
    batch_size = min(32, max(8, len(dataset) // 50))
    ...
    # Set fixed number of epochs
    num_epochs = 50
    
    print(f"Training for {num_epochs} epochs")
    
    # Initialize NutriNet model
    model = NutriNet(num_outputs=5).to(device)
    ...
    # Train model
    print("Starting NutriNet training...")
    ...
    model.load_state_dict(torch.load('nutrinet_checkpoint/best_nutrinet_model.pth'))
    print('[DEBUG] Training complete. Best model loaded. Proceeding to evaluation and specific dish test.')
    # Final evaluation with best model
    print("[DEBUG] About to run final evaluation block...")
    criterion = nn.MSELoss()
    print("\nFinal NutriNet Model Evaluation:")
    print("=" * 60)
    ...
    os.makedirs('nutrinet_result', exist_ok=True)
    save_evaluation_metrics(final_val_metrics, 'nutrinet_result/evaluation_metrics.txt', predictions=final_val_predictions, targets=final_val_targets, denorm_fn=dataset.denormalize_target)
    ...
    test_specific_dishes(model, dataset, device, output_file="nutrinet_result/specific_dish_test_results.txt")
